...
estado_pantalla estado=no_pulsada;

//Variables que indican la posición pulsada en la pantalla tactil
extern int tX;
extern int tY;
...
void cambiarEstado(estado_pantalla e){
	estado=e;
}

estado_pantalla estadoActual(){
	return estado;
}
...
void TSInt(void)
{
	...
	/*int tiempo=timer2_leer();
	while(timer2_leer()-tiempo<=10){}*/
	for( i=0; i<10; i++ )
	{
	...
	Pt[10] = (Pt[0] + Pt[1] + Pt[2] + Pt[3] + Pt[4] + Pt[5] + Pt[6] + Pt[7] + Pt[8] + Pt[9]) / 10;
	...
	/*tiempo=timer2_leer();
	while(timer2_leer()-tiempo<=30){}*/

	rI_ISPC = BIT_EINT2;            // clear pending_bit

	cambiarEstado(pulsada);
	tX = tmp;
	tY = Pt[10];
...
void Lcd_TC(void)
{
	...
	Lcd_Draw_Box(40,0,80,60,15);
	Lcd_Draw_Box(80,0,160,60,15);
	Lcd_Draw_Box(120,0,160,60,15);
	Lcd_Draw_Box(160,0,240,60,15);
	Lcd_Draw_Box(200,0,240,60,15);
	Lcd_Draw_Box(240,0,320,60,15);
	Lcd_Draw_Box(280,0,320,60,15);
